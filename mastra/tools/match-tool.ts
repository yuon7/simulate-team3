...
const MOCK_COMPANIES = [
  // 1: TechCorp Japan
  {
    ...
    salaryMax: 10000000,
    requiredSkills: [
      { name: "TypeScript", level: "必須" },
      { name: "React", level: "必須" },
      { name: "Node.js", level: "歓迎" },
    ],
  },
  // 2: WebSolutions Inc.
  {
  ...
  // 6: CloudNative Solutions
  {
    ...
    requiredSkills: [
      { name: "AWS", level: "必須" },
      { name: "Kubernetes", level: "必須" },
      { name: "Python", level: "歓迎" },
    ],
  },
...
export const matchTool = createTool({
  ...
  inputSchema: z.object({
    ...
    location: z
      .array(z.string())
      .optional()
      .describe('希望する勤務地のリスト。例: ["Tokyo", "Osaka"]'),
  ...
  execute: async ({ context }) => {
    ...
    // ソートしてAIに返す
    const sortedCompanies = scoredCompanies.sort(
