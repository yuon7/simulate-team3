...
export const calculateSkillScore = (
  ...
  if (requiredLevelSkills.length === 0) {
    ...
    // 歓迎スキルだけで判断
    const matchedRecommended = recommendedLevelSkills.filter((rs) =>
      userSkillSet.has(rs),
    );
  ...
  // 必須スキルが設定されている場合
  const matchedRequired = requiredLevelSkills.filter((rs) =>
    userSkillSet.has(rs),
  );
  ...
  // 必須スキルの達成度 (0.7点満点)
  const requiredScore =
    (matchedRequired.length / requiredLevelSkills.length) * 0.7;
  ...
  if (recommendedLevelSkills.length > 0) {
    ...
    recommendedScore =
      (matchedRecommended.length / recommendedLevelSkills.length) * 0.3;
  }
