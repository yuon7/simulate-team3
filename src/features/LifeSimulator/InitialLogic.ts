...
} from "@/features/LifeSimulator/InitialSchema";

export const ROOM_TYPES_DATA: Record<
  string,
  { label: string; baseCost: number }
> = {
...
export const getDistFactor = (
  ...
  const getGroup = (pref: string) => {
    ...
    if (
      ["青森県", "岩手県", "宮城県", "秋田県", "山形県", "福島県"].includes(
        pref,
      )
    )
      return "tohoku";
