...
import { Container, Group, Button, TextInput, Burger, Anchor, Drawer, Stack, NavLink, Text, Avatar, UnstyledButton, Divider } from "@mantine/core"
import { IconSearch, IconUser, IconLogout, IconBuilding, IconBriefcase, IconDeviceGamepad, IconHelp, IconInfoCircle, IconHome, IconList, IconMapPin } from "@tabler/icons-react"
...
export function HeaderContent({ user, avatarUrl }: HeaderContentProps) {
  ...
  const ProfileSection = () => {
    ...
    return (
      ...
      >
        <Group p="md">
          <Avatar src={avatarUrl} size="md" color="initials" name={user.name || user.email}>
            {!avatarUrl && <IconUser size={20} />}
          </Avatar>
          <div style={{ flex: 1 }}>
            <Text size="sm" fw={500}>{user.name || "ユーザー"}</Text>
        ...
        <Stack gap="xs">
          ...
          {!user && (
            <>
              ...
              />
              <Divider />
              <Button component="a" href="/auth/login" fullWidth variant="light" mt="md">
                ログイン
              </Button>
            </>
          )}
